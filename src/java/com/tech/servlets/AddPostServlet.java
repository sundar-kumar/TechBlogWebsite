...
public class AddPostServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
           ...
           String pcont=request.getParameter("post_cont");
           String pcode=request.getParameter("post_code");
           Part part=request.getPart("post_pic");
           HttpSession session=request.getSession();
           User user=(User)session.getAttribute("currentUser");
          Post post=new Post(ptitle, pcont, pcode, part.getSubmittedFileName(), null, cid, user.getId());
          PostDao dao=new PostDao(ConnectionProvider.getConnection());
          if(dao.savePost(post))
          {
              String path=request.getRealPath("/")+ "blog_pics" + File.separator + part.getSubmittedFileName();
              HelperProfile.savefile(part.getInputStream(), path);
              out.println("done");
          }
          else
          {
